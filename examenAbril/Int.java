...
public class Int 
{
    ...
    public boolean lessThan(Int that)
    {
        ...
        String menor = "no";
        int eq = 10;
        if(this.signo() == 1 && that.signo() == 1){signo = "positivo";}
        else if(this.signo() == -1 && that.signo() == -1){signo = "negativo";}
        else
        {
            if(this.signo() == -1)
            {
        ...
        if(signo == "negativo")
        {
            if(this.str_int.length() > that.str_int.length())
            {
                menor = "yes";
            }
            else if(this.str_int.length() < that.str_int.length())
            {
                menor = "no";
            }
            else{eq = 0;}
        ...
        if(eq == 0)//Caso para numeros de igual tamaño y mismo signo
        {
            StdOut.println("Sin implementar aun.");
        }
    ...
    public Queue<Int> getQueue()
    {
        ...
        if (this.signo == -1){cop_this.minus();}
        
        while(cop_this.equals(val))
        {
            queue.enqueue(val);
            val.plus(sum_1);
        }
    ...
    public static void main(String[] args)
    {
        Int n1 = new Int("21");
        Int n2 = new Int("3");
        //StdOut.println(n1.lessThan(n2));
        Queue<Int> q1 = n2.getQueue();
        for (Int i: q1){StdOut.println(i);}
    }
}